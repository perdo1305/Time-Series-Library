...
def organize_battery_data(input_dir=None, output_file=None):
    """
    ...
    3. Add a new column for capacity difference (now_capacity)
    4. Combine everything into a single output file
    """
    ...
    if output_file is None:
        output_file = os.path.join(input_dir, "organized_battery_data.csv")

    # Find all sheet2 CSV files
    csv_files = glob.glob(os.path.join(input_dir, "*_sheet2.csv"))
    ...
    for i, (csv_file, date) in enumerate(dated_files):
        ...
        try:
            ...
            # Calculate now_capacity as the difference between charge and discharge capacities
            df["now_capacity"] = (
                df["Charge_Capacity(Ah)"] - df["Discharge_Capacity(Ah)"]
            )
