...
class Dataset_Custom(Dataset):
    def __init__(
        ...
        if not self.use_json_selection and not os.path.exists(
            ...
            for subdir in os.listdir(self.root_path):
                full_path = os.path.join(self.root_path, subdir, self.data_path)
                if os.path.exists(full_path):
                    self.root_path = os.path.join(self.root_path, subdir)
                    found = True
                    break
            if not found:
                # Just use the first subdirectory if we can't find it
                subdirs = os.listdir(self.root_path)
                if subdirs:
                    self.root_path = os.path.join(self.root_path, subdirs[0])
    ...
    def __read_data__(self):
        ...
        # Filter out columns that don't provide useful information for prediction
        cols_to_remove = []
        if "Data_Point" in df_raw.columns:
            cols_to_remove.append("Data_Point")
        if "Is_FC_Data" in df_raw.columns:
            cols_to_remove.append("Is_FC_Data")
        if "source_file" in df_raw.columns:
            cols_to_remove.append("source_file")
        if "file_date" in df_raw.columns:
            cols_to_remove.append("file_date")
        if "Test_Time(s)" in df_raw.columns:
            cols_to_remove.append("Test_Time(s)")
        if "Cycle_Index" in df_raw.columns:
            cols_to_remove.append("Cycle_Index")
